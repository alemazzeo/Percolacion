...
for L in Ls:
    ...
    # Al hacer esto se recuperan automáticamente los resultados guardados.
    redes.append(perc(L))
...
ax1 = fig1.add_subplot(111, **kargs)
...
ax2 = fig2.add_subplot(111, **kargs)
...
# que percola sea 0.5 +/- la precision deseada
...
for i, red in enumerate(redes2):
    ...
    while abs(red.p - 0.5) > precision:
        ...
        prob.append(probabilidad)
        p.append(red.p)
    
    desviacion = abs((prob[-3]-prob[-2])/2)
    valor_pc = prob[-1]
    ...
    fig3[i] = plt.figure()
    ax = fig3[i].add_subplot(111, **kargs)
    ax.plot(prob, p, 'o', label='$F(p)\,dp$')  
